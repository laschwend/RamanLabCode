function [] = improvePlot()
    ...
    %set(hFig,'defaultAxesColorOrder',makePrettyColors());

    % Make the figure bigger
    set(hFig, 'rend', 'painters', 'Units', 'pixels', 'pos', ...
        [50 100 plot_width_in_px plot_height_in_px]);
    ...
    for i = 1:length(axis_handles)
        ...
        set(ax, 'FontSize', axis_tick_font_size, 'FontName', font_name, 'LineWidth', box_thickness);
        
        set(ax, 'Box', 'on');

        % Change font size for axis text labels
        set(get(ax, 'XLabel'),'FontSize', axis_label_font_size, 'FontWeight', 'Bold');
        set(get(ax, 'YLabel'),'FontSize', 10, 'FontWeight', 'Bold');
        set(get(ax, 'Title'),'FontSize', axis_label_font_size, 'FontWeight', 'Bold');
