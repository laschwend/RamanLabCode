# -*- coding: utf-8 -*-
"""
Created on Mon Jul 22 15:08:48 2024

@author: laSch

Purpose: to run the superviolin plotting code
"""
import io
import os
import pkgutil
...
def get_args(demonstration=False, preferences=False):
    ...
    if demonstration:
        ...
        if "args.txt" not in os.listdir():
            ...
            with open("C:/Users/laSch/Desktop/Raman Lab/RamanlabCode/superviolin-master/superviolin/superviolin/res/args.txt", "r") as f:
                lines = f.readlines()
            arg_dict = process_txt(lines)
            return arg_dict
...
class Superviolin:
    def __init__(self, condition="condition", value="value", replicate="replicate",
        ...
        self._on_legend = []
        print("this is the right superviolin code")
        if bw != "None":
            self.bw = bw
        else:
            self.bw = None
...
class Superviolin_OG:
    def __init__(self, condition="condition", value="value", replicate="replicate",
                 order="None", filename="", data_format="tidy",
                 centre_val="mean", middle_vals="mean", error_bars="SEM",
                 paired_data="no", stats_on_plot="no", ylimits="None",
                 total_width=0.3, linewidth=1, dataframe=False, dpi=300,
                 sep_linewidth=0.5, xlabel="", ylabel="", cmap="Set2",
                 bw="None", show_legend="no", return_stats=False):
        self.errors = []
        self.df = dataframe
    ...
    def generate_plot(self):
        ...
        """
        # if no errors exist, create the superplot. Otherwise, report errors
        click.echo("this is the right superviolin code")
        errors = self.check_errors()
        if not errors:
            self.get_kde_data(self.bw)
            self.plot_subgroups(self.centre_val, self.middle_vals,
    ...
    def plot_subgroups(self, centre_val="mean", middle_vals="mean", error_bars="SEM",
                       ylimits="None", total_width=0.5, linewidth=1,
                       show_stats="no", show_legend="no"):
        """
        Plot all subgroups of the df attribute
        ...
        if ylimits != "None":
            lims = (float(i) for i in ylimits.split(", "))
            plt.ylim(lims)
        plt.savefig('SupervioinHumanOrC2C12.pdf', dpi=300)
...
def make_superplot():
    ...
    click.echo("this is the right code")
    plt.savefig("C:/Users/laSch/Desktop/Raman Lab/RamanlabCode/superviolin-master/superviolin/superviolin/res/SuperviolinHumanOrC2c12.pdf", dpi=300)
