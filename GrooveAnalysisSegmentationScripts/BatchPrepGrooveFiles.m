...
%% loop through and generate all of the cross sections and save them and the angles to rotate by
...
for i = 1:length(fileNameData)
    ...
    img = getRotatedImageFromFile(fileNameData{i});
    ...
    c = 2*(q);
    subplot(n, 2, c+1);
    imshow(img./max(img,[],'all'),[])
    title(fileNameData{i}.name)
    mask = runSAMSegmentation(img);
    %mask = runThresholdSegmentation(img, 0.3);
    %mask = runAdaptiveSegementation(img, .15);
    %mask = runGlobalSegmentation(img,.2);
    
    img2 = uint8(img./max(img,[],'all')*255);
    
    subplot(n, 2, c+2);
    imshow(labeloverlay(img2, mask*2))
    
    fileNameData{i}.maskSAM = mask; 

end

%% save mask figures + the fileNameData
save_all_figures([segmentedLoc + "SAM\"])
...
function save_all_figures(foldername)
    % Save all open figures as PNG files in a specified folder
    % Usage: save_all_figures('my_folder')
    ...
    for i = 1:length(figures)
        ...
        % Create filename using figure number
...
function [thresMask] = runAdaptiveSegementation(img, thres)
     thresMask = adaptthresh(rescale(img), thres, 'NeighborhoodSize',11);
     figure()
     imshow(thresMask,[])
     thresMask = thresMask > thres; 
end

function [thresMask] = runThresholdSegmentation(img, thres)
    thresMask = imbinarize(rescale(img), 'global',thres);
end
function [thresMask] = runGlobalSegmentation(img, thres)
    thresMask = rescale(img) >thres; 
end
function [SAMMask] = runSAMSegmentation(img)
     % Open Image Segmenter app with the image
    % Load SAM model
    
    % If no points provided, let user click
    if nargin < 2 || isempty(points)
        fig = figure('Name', 'SAM Segmentation - forground click points', 'NumberTitle', 'off');
        imshow(img, []);
        title('Click on foreground objects to segment (press ENTER when done)');
        
        [x, y] = getpts(gca); % User clicks points
        points = [x, y];
        close(fig);

        fig = figure('Name', 'SAM Segmentation - background click points', 'NumberTitle', 'off');
        imshow(img, []);
        title('Click on background objects to segment (press ENTER when done)');
    ...
    try
        ...
        embeddings = extractEmbeddings(sam,img);
        imageSize = size(img);
        masks = segmentObjectsFromEmbeddings(sam,embeddings,imageSize, ...
    ForegroundPoints=points,BackgroundPoints=points2);
        SAMMask = masks;
        
    catch ME
        disp('Error during segmentation: ' + string(ME.message));
        SAMMask = [];
        return;
    end
